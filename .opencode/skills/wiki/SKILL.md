...
> Query **默认不记 log**；仅当去重结果显著影响生成决策（如避开大量已出题、调整选题方向）时可记一条 `## [yyyy-MM-dd] query | {topic}`。
...
3. **跨话题去重**：再读话题页末尾「关联」区的 `[[话题]]` 链接，对其中**已存在**的关联话题页，同样只读其题目索引表，纳入避重范围（防止跨话题重复出题，如 java基础 与 java并发 都可能出 ConcurrentHashMap）。
4. 若不存在：标记该话题为「新话题」，已登记题目为空。
5. 输出给生成环节：`{已出题目清单（来自索引表）, 关联话题已出题目清单, 已覆盖考点集合, 未覆盖考点（知识体系中有但无题的）, 当前最大题目序号 max}`。
   - 生成时严格避开已出题目（按摘要判断角度重复），优先覆盖未覆盖考点。
   - 生成环节按 max 预分配本批题目 ID（`{topic}-(max+1)` 起），写入 question 文件锚点，Ingest 时使用同一批 ID。
...
1. **确定每题 ID（优先锚点）**：读待评分的 question 文件，提取每题 `## Q` 标题下的 `<!-- id: {ID} -->` 隐藏锚点。若文件无锚点（2026-08 前的旧文件），回退为：Read `wiki/topics/{topic}.md` 的题目索引表（同 1a，只读上半部分），按摘要/考点与待评分题目对齐确定 ID。
...
1. 若 `wiki/topics/{topic}.md` 不存在：
   ...
   - 「题目登记」：写入本次 N 题，ID 从 `{topic}-001` 起（与 question 文件锚点一致）；每题节内追加 `#### 参考答案` 子节，写入该题参考答案。
2. 若存在：
   - Read 现有文件，取当前最大序号，新题 ID 从 `{topic}-{max+1}` 起续编。**若生成环节已预分配 ID**（question 文件每题含 `<!-- id: ... -->` 锚点），**必须使用同一批 ID**，确保锚点与登记一一对应。
   ...
   - 若本次题目触及「知识体系」中未列出的考点，补充进大纲。
    ...
    - **维护关联区**：若本次新题考点与其他**已存在**话题页明显重叠（如 java并发 与 java基础 都涉及 volatile），在双方「关联」区互加 `[[话题]]` 链接（只链接已存在页，避免断链）。
...
- **题目描述加粗**：话题页题目登记用 `- 题目：**{题目}**`，question 文件标题用 `## Q{n}：**{题目}**`，题库页题目正文加粗——便于阅读时快速定位题目。
