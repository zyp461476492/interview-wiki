...
2. 若存在：Read 它的**开头至 `## 题目登记` 标题前**（即 frontmatter + 知识体系 + 题目索引表）。可先 Grep `^## 题目登记` 定位分界行号，再用 Read offset/limit 截取上半部分；文件不大时直接 Read 全文但**只解析索引表 + 知识体系**。提取「题目索引表」全部条目（ID / 考点 / 摘要）与「知识体系」考点大纲。
3. 若不存在：标记该话题为「新话题」，已登记题目为空。
4. 输出给生成环节：`{已出题目清单（来自索引表）, 已覆盖考点集合, 未覆盖考点（知识体系中有但无题的）}`。
   - 生成时严格避开已出题目（按摘要判断角度重复），优先覆盖未覆盖考点。

### 1b. 答案检索模式（评分前）

> 渐进式加载：用 Grep 定位目标题节后分段 Read，不必全量加载整页。

步骤：

1. Read `wiki/topics/{topic}.md` 的题目索引表（同 1a，只读上半部分），按摘要/考点与本次待评分题目对齐，确定每题对应 ID。
2. 对每个待评分题：Grep `^### {ID}` 定位行号 -> Read 该 ID 节（offset=行号, limit≈20 行）取 `#### 参考答案`。
...
2. 检查项：
   ...
   - **索引表与题目登记不同步**：题目索引表的行数/ID/考点与题目登记实际条目不一致（多题、漏题、考点不符），修复为一致。
...
- **渐进式加载**：「题目索引表」是题目登记的轻量冗余摘要，去重 Query 只读索引表 + 知识体系（不读详情）；答案检索用 Grep `^### {ID}` 定位详情后分段 Read。Ingest/Lint 须保持索引表与题目登记同步。
