...
| 操作       | 时机                              | 简述                                                                                          |
| ---------- | --------------------------------- | --------------------------------------------------------------------------------------------- |
| **Query**  | 生成面试题**之前**；评分**之前**  | 去重模式：读 `wiki/index.md` + `wiki/topics/{topic}.md`，取已登记题目与已覆盖考点，生成时避开。答案检索模式：读话题页取回已登记 `#### 参考答案` 作为评分基准。**未 Query 不生成。** |
| **Ingest** | 生成题目**之后**（及评分后、用户放入 `raw/` 源材料时）      | 创建/更新 `wiki/topics/{topic}.md`（知识体系 + 题目登记 + **参考答案**），更新 `wiki/index.md`，追加 `wiki/log.md`；评分后若有更优答案则更新对应题；raw 源材料 Ingest 为可选支线（补知识体系考点、log 记来源）。**未 Ingest 视为流程未完成。** |
| **Lint**   | 用户请求时                        | 检查重复/近似题、参考答案缺失、覆盖缺口、孤立页、索引不同步，报告并修复。                                   |
